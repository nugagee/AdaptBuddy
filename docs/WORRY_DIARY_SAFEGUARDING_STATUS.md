...
| Journal, signal, and alert storage | Three separate browser-initiated writes exist | Must become one authenticated, server-side transaction with idempotency and failure handling |
...
| Accepted trusted adult | Source guardrails and a draft SQL design exist | Invitation delivery and accept/decline UI are missing; SQL is unapplied and unverified |
...
These source changes still need a full application build, preview review, and multi-account integration tests. Static checks do not prove runtime safeguarding or database authorisation.
...
1. Journal entry, assessment, incident creation, and recipient fan-out are not atomic or server-owned.
...
4. Urgent child-facing guidance is not integrated into the Worry Diary disclosure flow.
5. Historical class/teacher RLS and RPCs and the teacher-verification lifecycle are incomplete.
6. There are no full cross-account tests for a child, accepted parent, unrelated adult, approved teacher, and unapproved teacher.
