...
**Current enablement release:** the owner requested adult invitations and support recording, confirmed the administrator identity, and confirmed password rotation. The narrow 041 authority boundary and scoped 044 support contracts are now applied and verified on the confirmed hosted project. The enabled application passed its local checks and is prepared for production deployment; see [SCOPED_SUPPORT_ENABLEMENT.md](SCOPED_SUPPORT_ENABLEMENT.md). Historical school/guardian authority and safeguarding operations are not marked complete.
...
| --- | --- | --- |
| R1 | Reconcile and release database contracts | Reviewed profile, mood privacy, pending invitation and atomic support changes are applied in a tested order; ordinary profile saves and both new request calls succeed on the intended environment. Scoped mood/profile fields are now live. The narrow 041/044 authority/support contracts are now applied; deployed definitions/grants and preserved counts match the local rehearsal. |
| R2 | Establish administrator authority | Complete for this release: owner confirmed identity and password rotation, the intended administrator is verified, and zero old sessions remained at preflight/readback. Evidence contains no credentials. |
