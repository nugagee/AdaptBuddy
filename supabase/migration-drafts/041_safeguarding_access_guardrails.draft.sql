...
begin
  if new.adult_id is null
     or (old.adult_id is not null and (new.adult_id is distinct from old.adult_id
...
create trigger trusted_adults_demote_on_identity_change
  before update of adult_id, email, role, child_id on public.trusted_adults
...
-- Central invalidation covers child revocation, adult unlink, direct deletion,
-- changed identities and withdrawal of guardian evidence.
create or replace function public.invalidate_guardian_class_access(p_child_id uuid, p_adult_id uuid)
returns void language plpgsql security definer set search_path = public
as $$
begin
  update public.class_memberships membership set status = 'paused', updated_at = now()
  where membership.child_id = p_child_id and membership.status = 'active'
    and exists (select 1 from public.class_join_requests request
      where request.class_id = membership.class_id and request.child_id = membership.child_id
        and request.parent_approved_by = p_adult_id);
  update public.class_join_requests set parent_approved = false, parent_approved_by = null,
    parent_approved_at = null, teacher_approved = false, teacher_approved_by = null,
    teacher_approved_at = null, approved_at = null,
    status = case when status in ('declined', 'cancelled') then status else 'pending' end,
    updated_at = now()
  where child_id = p_child_id and parent_approved_by = p_adult_id;
end;
$$;
revoke all on function public.invalidate_guardian_class_access(uuid, uuid) from public, anon, authenticated;
...
begin
  if old.adult_id is null then return coalesce(new, old); end if;
  if tg_op = 'DELETE' then
    acceptance_ended := true;
    authority_changed := true;
  else
    acceptance_ended := new.adult_id is distinct from old.adult_id
      or new.child_id is distinct from old.child_id
      or new.role is distinct from old.role or new.email is distinct from old.email
      or new.status not in ('active', 'connected') or new.accepted_at is null
      or new.accepted_by is distinct from new.adult_id
      or new.acceptance_method is distinct from 'account_email';
    authority_changed := acceptance_ended
      or new.guardian_verified_at is distinct from old.guardian_verified_at
      or new.guardian_verified_by is distinct from old.guardian_verified_by
      or new.guardian_verification_method is distinct from old.guardian_verification_method;
  end if;
  if authority_changed then
    perform public.invalidate_guardian_class_access(old.child_id, old.adult_id);
  end if;
  if acceptance_ended then
    delete from public.child_relationships where parent_id = old.adult_id and child_id = old.child_id;
  end if;
...
create or replace function public.record_trusted_adult_request_v1(
  p_name text,
  p_role text,
  p_email text,
  p_phone text,
...
begin
  if v_child_id is null or v_child_id is distinct from p_child_id then
  ...
  into v_child_role
  from public.profiles
  where id = v_child_id and is_authorized is true and status = 'active';
  ...
  if v_existing.id is not null
    ...
    raise exception 'This adult already has a connection. Manage that connection separately.';
  end if;
...
revoke all on function public.record_trusted_adult_request_v1(text, text, text, text, uuid) from public, anon;
grant execute on function public.record_trusted_adult_request_v1(text, text, text, text, uuid) to authenticated;

create or replace function public.add_trusted_adult_for_child(p_name text, p_role text, p_email text, p_phone text)
returns public.trusted_adults language plpgsql security definer set search_path = public
as $$ begin
  raise exception using errcode = '42501', message = 'Update AdaptBuddy before recording a trusted-adult request.';
end; $$;
revoke all on function public.add_trusted_adult_for_child(text,text,text,text) from public, anon, authenticated;
...
-- Removing/suspending an account or changing a class owner invalidates derived access.
create or replace function public.invalidate_profile_support_access()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
  if tg_op = 'DELETE' or new.role is distinct from old.role or new.email is distinct from old.email
    ...
    update public.class_join_requests set parent_approved = false, parent_approved_by = null,
      ...
      status = case when status in ('declined', 'cancelled') then status else 'pending' end,
      updated_at = now()
      where teacher_approved_by = old.id or child_id = old.id;
  end if;
  if tg_op = 'DELETE' then return old; end if;
  return new;
end;
$$;
revoke all on function public.invalidate_profile_support_access() from public, anon, authenticated;
drop trigger if exists profiles_invalidate_support_access on public.profiles;
create trigger profiles_invalidate_support_access before update or delete on public.profiles
for each row execute function public.invalidate_profile_support_access();

create or replace function public.invalidate_class_owner_access()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
  if new.teacher_id is distinct from old.teacher_id then
    update public.class_memberships set status = 'paused', updated_at = now() where class_id = old.id;
    update public.class_join_requests set parent_approved = false, parent_approved_by = null,
      parent_approved_at = null, teacher_approved = false, teacher_approved_by = null,
      teacher_approved_at = null, approved_at = null,
      status = case when status in ('declined', 'cancelled') then status else 'pending' end,
      updated_at = now() where class_id = old.id;
  end if;
  return new;
end;
$$;
revoke all on function public.invalidate_class_owner_access() from public, anon, authenticated;
drop trigger if exists teacher_classes_invalidate_owner_access on public.teacher_classes;
create trigger teacher_classes_invalidate_owner_access before update of teacher_id on public.teacher_classes
for each row execute function public.invalidate_class_owner_access();
